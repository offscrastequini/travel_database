...
Execute the script using MySQL Workbench or your preferred MySQL client.

The script will:

1. Create the `viagens` database.
2. Create the `users`, `destinos`, and `reservas` tables.
3. Define primary and foreign keys.
4. Configure constraints and cascading relationships.
5. Insert sample users and destinations.
6. Insert sample reservations.
7. Execute filtering and relational queries.
8. Update existing records.
9. Add additional address fields to users.
10. Create indexes.
11. Demonstrate subqueries and aggregate functions.
12. Use `EXPLAIN` to analyze query execution.
...
The SQL script also includes **30 additional users** to provide a larger dataset for testing queries, indexes, and execution plans.
